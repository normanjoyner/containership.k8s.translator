...
function csApplicationToK8SReplicationController(csAppDesc) {
    // defines default kubernetes application properties
    const k8sAppDefaults = {
        env_vars: {
            CS_ORCHESTRATOR: 'kubernetes',
            PORT: csAppDesc.container_port
        }
    };

    // merge default kubernetes applicaiton properties and default containership application properties
    _.defaultsDeep(k8sAppDefaults, constants.application.SCHEMA);

    // enforce application defaults
    _.defaultsDeep(csAppDesc, k8sAppDefaults);
...
function csApplicationFromK8SReplicationController(k8sRC) {
    ...
    return _.merge(
        {
            // default to false, if k8s spec is not privileged then nested
            // securityContext does not exist so this will be undefined
            privileged: false
        },
